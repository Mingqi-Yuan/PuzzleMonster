...
class NPUZZLE:
    def __init__(
        ...
        self.org_mat = np.arange(0, self.total_num).reshape((self.grid_x, self.grid_y))
        self.cur_mat = None
    ...
    def get_feasible_action(self, blank_loc):
        ...
        if blank_loc == [0, 0]:
            ...
            feasible_action = [1, 3]
        elif blank_loc == [self.grid_x-1, 0]:
            ''' bottom left corner, swap with right and top '''
            feasible_action = [1, 2]
        elif blank_loc == [0, self.grid_y-1]:
            ''' top right corner, swap with left and bottom '''
            feasible_action = [0, 3]
        elif blank_loc == [self.grid_x-1, self.grid_y-1]:
    ...
    def get_reward(self):
        ...
        for i in range(self.total_num):
            org_ploc = np.where(self.org_mat == i)
    ...
    def is_done(self):
        if (self.cur_mat == self.org_mat).all():
            self.done = True
        else:
            self.done = False
